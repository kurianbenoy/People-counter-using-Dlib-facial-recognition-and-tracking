...
##########detecting#################################################
  ...
  #############Processing###########################################
        ...
        for (objectID, centroid) in objects.items():
            ...
            if centroid[2] == 'Unknown' and frame_cnt%skip_frame == 0 and det_flag ==1 and abs(h-w)<3:
                if img_count[objectID] < train_cnt:
                    ...
                    Unknown_images_bbox[objectID][img_count[objectID]] = [
                        int(centroid[4]), int(centroid[5]), int(centroid[6]), int(centroid[3])]
                    img_count[objectID] += 1

            cv2.putText(frame, str(centroid[2]), (int(centroid[0]), int(
                centroid[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 0), 2)
            cv2.rectangle(frame, (int(centroid[3]), int(centroid[4])), (int(
                centroid[5]), int(centroid[6])), (0, 0, 0), 2)
            ...
            if img_count[objectID] == train_cnt and enter == 0:
                ...
                for k in range(10):
                    ...
                    roi = a[Unknown_images_bbox[objectID][k][0]:Unknown_images_bbox[objectID][k]
                            [2], Unknown_images_bbox[objectID][k][3]:Unknown_images_bbox[objectID][k][1]]
                    try:
        ...
        if img_count[i] == train_cnt:
            box = [(Unknown_images_bbox[i][j][0], Unknown_images_bbox[i][j][1],
                    Unknown_images_bbox[i][j][2], Unknown_images_bbox[i][j][3])]
...
def retinanet():
    ...
    model_path = os.path.join(
        '.', 'snapshots', 'head_detection_model_res50.h5')
